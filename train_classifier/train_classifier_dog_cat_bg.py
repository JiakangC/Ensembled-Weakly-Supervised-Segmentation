...
def train_classifier(model,train_loader, test_loader, device, num_epochs=3, learning_rate=0.0001):
    ...
    for epoch in range(num_epochs):
        ...
        for i,data in enumerate(train_loader):
            ...
            labels = torch.tensor([0 if label in ([ 0,  5,  6,  7,  9, 11, 20, 23, 26, 27, 32, 33]) else 2 if label == 37 else 1 for label in labels])
...
if __name__ == "__main__":
    ...
    train_loader = dataloader.get_train_loader_label()
    ...
    torch.save(best_model_param, '../train_classifier/model_saved/model_saved/resnet50_dog_cat_bg.pth')